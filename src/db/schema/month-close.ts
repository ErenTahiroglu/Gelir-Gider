...
export const monthCloseRevisions = pgTable(
	...
	(table) => [
		...
		check(
			...
			sql`(${table.route} = 'SHORT_TERM_GOAL') = (${table.targetGoalId} IS NOT NULL) AND (${table.route} = 'SHORT_TERM_GOAL') = (${table.targetGoalRevisionNo} IS NOT NULL) AND (${table.route} = 'SHORT_TERM_GOAL') = (${table.targetBucketId} IS NOT NULL)`,
		),
		check(
