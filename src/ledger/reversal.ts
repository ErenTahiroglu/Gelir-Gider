...
export async function reverseJournalEntry({
	...
	occurredAt,
	memo,
}: ReverseJournalEntryParams): Promise<ReverseJournalEntryResult> {
	// 1. Input validation
	...
	// 2. Execute in a single database transaction
	return await db.transaction(async (tx) => {
		// 2.1 Early Idempotency Fast Path
		...
		// 2.2 Lock Original Entry (FOR UPDATE)
		...
		// 2.4 Load original lines in deterministic order
		const originalLines = await tx
			.select({
				lineNo: journalLines.lineNo,
				accountId: journalLines.accountId,
		...
		// 2.5 Invert original lines (exact swap of debit and credit)
		...
		// 2.6 Compute reversal fingerprint
		...
		try {
			...
			// Catch partial unique index violation on reversal_of_entry_id if concurrent transaction raced
			if (
		...
		// If idempotency conflict occurred concurrently
		if (!insertedDraft) {
		...
		// 2.9 Transition DRAFT -> POSTED (triggers DB verification of exact inverse lines)
		try {
